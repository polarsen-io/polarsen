...
(
    ...
    name          TEXT        NOT NULL CONSTRAINT name_valid CHECK ( length(name) < 1000 ),
    internal_code TEXT        NOT NULL,
    created_at    timestamptz NOT NULL DEFAULT now(),
    meta          JSONB,

    CONSTRAINT unique_message_group_method_idx UNIQUE (internal_code)
);
...
(
    ...
    created_at      timestamptz NOT NULL DEFAULT now(),
    summary         TEXT        CONSTRAINT summary_valid CHECK (length(summary) < 10000),
    title           TEXT        CONSTRAINT title_valid CHECK (length(title) < 1000),
    meta            JSONB,
    run_id          UUID,
    chat_id         BIGINT      NOT NULL REFERENCES general.chats,

    CONSTRAINT unique_message_group_idx UNIQUE (group_method_id, internal_code)
);
...
(
    ...
    CONSTRAINT unique_message_group_chat_idx UNIQUE (group_id, msg_id)
);
